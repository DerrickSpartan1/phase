...
- **Prefer `oracle_nom` combinators** — all parser branches delegate to `parser/oracle_nom/` for shared parsing operations: numbers (`parse_number`, `parse_number_or_x`), mana (`parse_mana_symbol`, `parse_mana_cost`), colors (`parse_color`), P/T (`parse_pt_modifier`), roman numerals (`parse_roman_numeral`), counter types (`parse_counter_type`), and medium-level patterns (conditions, durations, quantities, target filters). The nom combinators provide structured `VerboseError` traces and word-boundary safety (e.g., `parse_article_number` prevents "another" matching as "a"). Use `parse_number_or_x` when X represents a variable resolving to 0 (costs, P/T, counters); use `parse_number` when X should remain as `Variable("X")` (effect quantities). `dispatch_line_nom` in `oracle.rs` is the primary dispatch path for lines not caught by earlier priorities — it calls `parse_effect_chain_with_context` for effect candidates, with structural classification fallback for diagnostic traces. All parser branch files use nom combinators for dispatch. New parser code MUST use nom combinators — `starts_with`/`strip_prefix` for parsing dispatch is not acceptable. Use `nom_on_lower` bridge for mixed-case text, `tag().parse()` for already-lowercase text.
